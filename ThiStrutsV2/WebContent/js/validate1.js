function sumTotal(){
	...
	var soLuongKheCamRam = giaMain1[3];
	var dungLuongToiDa = giaMain1[4];
	...
	var dungLuong = giaRamChuyen[3];
	//lay value so luong ram
	var soLuongRam = $('.SoLuong').val();
	//tinh tong tien ram theo so luong ram
	var giaRamTong = Number(giaRamChuyen2) * Number(soLuongRam);
	//tinh tong dung luong cua ram
	var dungLuongRam = Number(dungLuong) * Number(soLuongRam);
	//so sanh tong dung luong cua mainboard va dung luong cua ram
...
//	$('#total').text(total);
	...
	//so sanh ram va mainboard
	if(loaiRam != null && loaiRam != loaiRamMain && loaiRam != "" && loaiRamMain != null && loaiRamMain != ""){
		alert('Bạn chọn Ram và Mainboard khác loại nhau!!!Xin mời chọn lại');
		$('#Ram').val("0-null");
	}
	//so sanh so luong ram
	if(soLuongRam !=null && soLuongRam > soLuongKheCamRam && soLuongRam != "" && soLuongKheCamRam != null && soLuongKheCamRam !=""){
		alert('bạn chọn quá số lượng ram mà mainboard có thể chứa!!!Xin mời chọn lại');
		$('#SoLuong').val("");
	}
	//so sanh dung luong ram
	if(dungLuongRam > dungLuongToiDa){
		alert('Bạn chọn quá dung lượng tối đa Ram mà Mainboard cho phép');
